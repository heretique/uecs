...
describe("examples", function () {
  ...
  it(".emplace example 2", function () {
    ...
    expect(() => world.emplace(0, new A())).toThrowError(
      new Error(`Cannot set component "${A.name}" for dead entity ID 0`)
    );
  });

  it(".remove example", function () {
    class A {
      constructor(public value = 10) {}
    }
    const world = new World();
    const entity = world.create();
    world.emplace(entity, new A());
